...
public class MiniRoomWindow implements Initializable {
	...
	public int getMovieShowPosition() {
		...
		for (int i = 0; i < CineController.shows.size(); i++) {
			if (CineController.shows.get(i).equals(movieShow)) {
				movieId = i;
				break;
			}
		}
	...
	void assignChair(ActionEvent event) throws EmptyFieldsException, UnselectedChairException {
		...
		if (!validReg) {
			...
			CineController.shows.get((getMovieShowPosition())).addCustomer(position,
	...
	public int[] getChairPosition(ActionEvent event) {
		...
		resetButtonConfig(CineController.shows.get((getMovieShowPosition())));
